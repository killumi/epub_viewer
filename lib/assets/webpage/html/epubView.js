...
// Zoom gesture tracking variables
var zoomGestureActive = false;
var initialZoomDistance = 0;
var currentFontSize = 16; // Default font size in px
var minFontSize = 10;
var maxFontSize = 32;
var lastZoomTime = 0;
var zoomThrottle = 50; // ms between zoom updates
var baseFontSize = 16; // Store the font size when zoom started
...
// Function to update font size with throttling
function updateFontSize(newSize, force) {
  if (!rendition) return;

  var now = Date.now();

  // Throttle updates unless forced
  if (!force && (now - lastZoomTime) < zoomThrottle) {
    return;
  }

  lastZoomTime = now;

  // Clamp font size to min/max limits
  newSize = Math.max(minFontSize, Math.min(maxFontSize, newSize));

  // Round to avoid micro-changes
  newSize = Math.round(newSize * 10) / 10;

  if (Math.abs(newSize - currentFontSize) > 0.05 || force) {
    currentFontSize = newSize;

    // Update font size in rendition
    rendition.themes.fontSize(newSize + 'px');

    // Call Flutter callback (throttled)
    try {
      console.log('Calling onFontSizeChanged with:', currentFontSize);
      if (window.flutter_inappwebview && window.flutter_inappwebview.callHandler) {
        window.flutter_inappwebview.callHandler('onFontSizeChanged', currentFontSize);
      } else if (window.parent && window.parent.flutter_inappwebview && window.parent.flutter_inappwebview.callHandler) {
        window.parent.flutter_inappwebview.callHandler('onFontSizeChanged', currentFontSize);
      }
    } catch (err) {
      console.error('Error calling onFontSizeChanged:', err);
    }
  }
}

// Public function to set font size externally
function setFontSize(fontSize) {
  updateFontSize(fontSize, true);
  baseFontSize = fontSize;
}

function loadBook(data, cfi, initialXPath, manager, flow, spread, snap, allowScriptedContent, direction, useCustomSwipe, backgroundColor, foregroundColor, fontSize, clearSelectionOnNav, selectAnnotationRangeParam, customCss, minFontSizeParam, maxFontSizeParam) {
  ...
  // Set font size limits and reset zoom state
  minFontSize = minFontSizeParam !== undefined ? minFontSizeParam : 10;
  maxFontSize = maxFontSizeParam !== undefined ? maxFontSizeParam : 32;
  currentFontSize = fontSize !== undefined ? fontSize : 16;
  baseFontSize = currentFontSize; // Initialize baseline
  ...
  rendition.on("relocated", function (location) {
    ...
    //
    // NOTE: In scrolled flow mode the relocated event fires from the user's native scroll
    // gesture, not from programmatic navigation. Fast scrolling can cause the browser to
    // briefly trigger text-selection state (lastCfiRange / isSelecting), making
    // hasActiveSelection() return true with no real selection. Reverting here would teleport
    // the reader back to lastValidLocation on every fast cross-chapter scroll.
    // In scrolled mode we clear any stale selection state and skip the revert entirely —
    // programmatic navigation is already protected by the overridden rendition methods.
    if (flow === 'scrolled' && hasActiveSelection() && !isSelecting) {
      isSelecting = false;
      lastCfiRange = null;
    }
    if (flow !== 'scrolled' && hasActiveSelection() && !isSelecting) {
  ...
  rendition.hooks.content.register((contents) => {
    ...
    var touchStartHandler = function (e) {
      ...
      // Handle zoom gesture with two fingers ONLY
      if (e.touches && e.touches.length >= 2) {
        zoomGestureActive = true;
        initialZoomDistance = getTouchDistance(e.touches[0], e.touches[1]);
        baseFontSize = currentFontSize; // Store current font size as baseline

        // Prevent all default behaviors for multi-touch
        e.preventDefault();
        e.stopPropagation();
        e.stopImmediatePropagation();
        return false;
      } else if (e.touches && e.touches.length === 1) {
        // Single touch - only reset zoom if it was previously active
        if (zoomGestureActive) {
          zoomGestureActive = false;
          initialZoomDistance = 0;
        }
        // Don't prevent default for single touches - let normal navigation work
      }
    ...
    doc.addEventListener('touchmove', function (e) {
      // Handle zoom gesture with two fingers
      if (zoomGestureActive && e.touches && e.touches.length === 2) {
        var currentDistance = getTouchDistance(e.touches[0], e.touches[1]);

        // Avoid division by zero and very small distances
        if (initialZoomDistance > 20) {
          var scaleFactor = currentDistance / initialZoomDistance;

          // Limit scale factor to reasonable range
          scaleFactor = Math.max(0.5, Math.min(2.0, scaleFactor));

          // Calculate new font size based on original baseline, not current
          var fontRange = maxFontSize - minFontSize;
          var relativeSize = (baseFontSize - minFontSize) / fontRange;
          var scaledRelativeSize = relativeSize * scaleFactor;

          // Apply scaling with reduced sensitivity
          var newFontSize = minFontSize + (scaledRelativeSize * fontRange);
    ...
    var touchEndHandler = function (e) {
      ...
      if (zoomGestureActive) {
        zoomGestureActive = false;
        initialZoomDistance = 0;
        baseFontSize = currentFontSize; // Update baseline for next zoom

        // Force final font size update
        updateFontSize(currentFontSize, true);
      ...
      // Check if this was a tap gesture (regardless of selection state)
      var wasTap = false;
      if (touchStartX !== null && currentTouchX !== null && touchStartTime !== null) {
        var deltaX = Math.abs(currentTouchX - touchStartX);
        var deltaY = Math.abs(currentTouchY - touchStartY);
        var touchDuration = Date.now() - touchStartTime;
        wasTap = (!hasMoved && deltaX < 10 && deltaY < 10 && touchDuration < 300);

        // If it was a tap and we have coordinates, call onTap handler
        if (wasTap && tapCoords) {
          try {
            callFlutterHandler('onTap', tapCoords.x, tapCoords.y);
          } catch (tapErr) {
            // Ignore tap handler errors
          }
        }
      }
...
function detectSwipe(el, func) {
  ...
  ele.addEventListener('touchmove', function (e) {
    ...
    if (hasActiveSelection()) {
      ...
      swipe_det.eX = t.screenX;
      swipe_det.eY = t.screenY;
      // Only preventDefault for horizontal swipes so native vertical scroll still works
      var dX = Math.abs(swipe_det.eX - swipe_det.sX);
      var dY = Math.abs(swipe_det.eY - swipe_det.sY);
      if (dX > dY) {
        e.preventDefault();
      }
    }
